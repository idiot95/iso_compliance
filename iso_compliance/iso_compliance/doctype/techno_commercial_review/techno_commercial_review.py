...
OVERALL_PARAMETERS = (
	...
	"Deviations from customer requirements identified",
	"Clarifications required from customer",
	"Technical and commercial conditions acceptable",
	"Overall techno-commercial feasibility confirmed",
)
...
class TechnoCommercialReview(Document):
	...
	def require_complete_checklist(self):
		unanswered = [
			row.parameter
			for field, _p in self._parameter_sets()
			for row in (self.get(field) or [])
			if not row.response
		]
		if unanswered:
			frappe.throw(
				_("Answer every review parameter (Yes / No / NA) before completing the review. Unanswered: {0}").format(
